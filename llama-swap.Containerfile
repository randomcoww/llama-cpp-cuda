...
RUN set -x \
  ...
  \
  && TINI_VERSION=$(curl -fsSL https://api.github.com/repos/krallin/tini/releases/latest | grep tag_name | cut -d '"' -f 4) \
  && curl -fsSL -o /tini \
    https://github.com/krallin/tini/releases/download/${TINI_VERSION}/tini-${TARGETARCH} \
  && chmod +x /tini \
  \
  && VERSION=$(curl -fsSL https://api.github.com/repos/mostlygeek/llama-swap/releases/latest | grep tag_name | cut -d '"' -f 4 | tr -d 'v') \
  && curl -fsSL -o llama_swap.tar.gz \
